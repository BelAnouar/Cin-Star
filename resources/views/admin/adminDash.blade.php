...
<body>
    ...
    <body>
        ...
        <!-- Edit Modal HTML XXXX-->
        <form {{ route('movie.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
        ...
        <!-- Edit Modal HTML -->
        <!-- @dd($movies) -->
        <form action="{{ route('movie.update', ['id' => $movies->id]) }}" method="POST">
            @csrf
            @method('PUT')
